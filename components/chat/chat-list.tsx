...
const ContactSidebar: FC<ContactSidebarProps> = ({
	...
	const handleSearchChange = (value: string) => {
		...
		// If search is empty, show normal list
		if (!value.trim()) {
			setShowSearchResults(false);
			setSearchResults([]);
			return;
	...
	const handleStartChatWithFriend = async (userId: string) => {
		try {
	...
	// Filter chats và requests dựa trên searchQuery
	...
	return (
		...
		>
			...
			{showSearchResults && searchResults.length > 0 && (
				<div className="space-y-2">
					...
					{searchResults.map((user) => (
						<div
							key={user.id}
							className="p-3 rounded-lg flex items-center gap-3 bg-white/10"
						>
							...
							<div className="flex items-center gap-2">
								{user.isFriend ? (
									<>
										<span className="text-xs text-green-400">✓ Bạn bè</span>
										<Button
											size="sm"
											variant="ghost"
											onClick={() => handleStartChatWithFriend(user.id)}
									...
									<span className="text-xs text-yellow-400">⏳ Đã gửi lời mời</span>
								) : (
									<Button
										size="sm"
			...
			{/* Danh sách chat và requests như cũ */}
			{!showSearchResults && (
				<>
					{activeTab === "friends" && filteredChats.length === 0 && (
						<div className="text-center text-white/50 py-8">
							{searchQuery
