...
const CACHE_KEY = "history_data";

// --- Main Component ---
// Move client hooks into an inner client component and wrap with Suspense in the default export
function HistoryInner() {
  // Initialize from cache
  const cached = cacheUtils.get<HistoryCache>(CACHE_KEY);

  const [sessions, setSessions] = useState<StudySession[]>(cached?.sessions || []);
  const [analytics, setAnalytics] = useState<Analytics | null>(cached?.analytics || null);
  const [viewMode, setViewMode] = useState<"day" | "week" | "month">("day");
  const [loading, setLoading] = useState(!cached);
  const [completedTasks, setCompletedTasks] = useState(cached?.analytics?.totalSessions || 0);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      let newAnalytics: Analytics | null = null;
      ...
      if (analyticsRes.ok) {
        ...
        newAnalytics = data.analytics;
        setAnalytics(newAnalytics);
        setCompletedTasks(newAnalytics?.totalSessions || 0);
      }

      // Cache the data
      cacheUtils.set<HistoryCache>(CACHE_KEY, {
